...
static int netl_handler(struct netl_handle *h,
	...
	case RTM_DELLINK:
		{
			...
			if (rta_tb[IFLA_ADDRESS])
				memcpy(&lladdr, RTA_DATA(rta_tb[IFLA_ADDRESS]),
					   sizeof(lladdr));
...
struct netl_handle *netl_create(unsigned events)
{
	...
	netl_handle =
		calloc(1, sizeof(struct netl_handle));
	if (netl_handle == NULL)
		return NULL;
